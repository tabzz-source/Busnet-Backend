...
const CUSTOMER_PROFILE_FOLDER =
  process.env.CLOUDINARY_PROFILE_FOLDER || "busnet/customers/profiles";
...
const changePassword = async (accountId, currentPassword, newPassword) => {
  if (!accountId || !currentPassword || !newPassword) {
    throw new AppError(
      "Account id, current password, and new password are required",
      400,
    );
  }
...
const updateMyProfile = async (accountId, data, file) => {
  ...
  if (account.role !== "CUSTOMER") {
    throw new AppError(
      "This endpoint is only available for customer accounts",
      403,
    );
  }
...
//   }
  ...
  if (file) {
    try {
      ...
      throw new AppError("Upload profile picture failed", 500);
    }
  }
